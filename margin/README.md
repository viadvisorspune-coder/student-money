...
- `css/fonts.css` and `fonts/` hold Cal Sans (headers and sub-headers) and Nunito Sans 400, 500, 600, italic (body), self-hosted under the SIL Open Font Licence. Cal Sans has no ₹ sign; inside the Cal Sans family that one character is drawn from Nunito Sans 700.
...
- **Typeface.** Nunito Sans stands in for Avenir Next, which is not free to embed. If the report is licensed for Avenir Next, swap it in `tokens.json`.
